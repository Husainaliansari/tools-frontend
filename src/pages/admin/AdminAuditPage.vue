<script setup lang="ts">
  ...
  function iconFor(action: string): string {
    ...
    if (action.startsWith('tool.')) return 'grid'
    if (action.startsWith('settings.')) return 'settings'
    if (action.startsWith('file.')) return 'folder'
    if (action.startsWith('job.')) return 'activity'
    return 'history'
  }
</script>
...
<template>
  <div class="ad-grid ad-fade-up">
    <AdminPageHeader title="Audit Logs" subtitle="Complete audit trail of admin actions." />
    ...
    <AdminCard>
      ...
      <div v-else-if="!rows.length" class="state ad-muted">No audit events recorded.</div>
      <template v-else>
        <div
          v-for="(log, i) in rows"
          :key="log.id"
        ...
        >
          ...
          <div class="feed__body">
            ...
            <div v-if="log.actor_email" class="feed__actor">{{ log.actor_email }}</div>
          </div>
...
<style scoped>
  ...
  .feed__actor { font-size: 12px; color: var(--ad-text2); margin-top: 3px; }
